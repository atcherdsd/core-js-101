...
function memoize(func) {
  const cache = {};
  return () => {
    cache.func = cache.func ? cache.func : func();
    return cache.func;
  };
}
...
function retry(func, attempts) {
  let maxCallCount = attempts;
  return function retry2() {
    if (func) {
      try {
        return func();
      } catch (error) {
        maxCallCount -= 1;
        return retry2(func, maxCallCount);
      }
    }
    return func;
  };
...
function logger(func, logFunc) {
  return function fn(...args) {
    const { name } = func;
    logFunc(`${name}(${JSON.stringify(args).slice(1, -1)}) starts`);
    const work = func(...args);
    logFunc(`${name}(${JSON.stringify(args).slice(1, -1)}) ends`);
    return work;
  };
}
